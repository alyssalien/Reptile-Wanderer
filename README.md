...
每個物種有偏好的棲地。不符合的地點**不會被排除**,而是給予評分懲罰、排到後面 —— 最適合的棲地浮在最前,但結果永遠不會是空的:

| 物種 | 偏好(排前面) | 不適合(排後面) |
|------|--------------|------------------|
| 🦎 守宮/蜥蜴(夜行性) | **有遮蔭**(遮蔭公園、林蔭步道) | 無遮蔭(高溫/強光更扣分) |
| 🐢 陸龜(平坦地形) | **草地、平坦公園** | 崎嶇森林、校園、商店 |
| 🦕 鬃獅蜥(安靜區域) | 公園、草地、森林、校園 | 繁忙便利商店 |

> ⚠️ **階梯限制**:陸龜「不可走階梯」目前僅以「平坦地點偏好」近似達成。真正在路徑層級避開 `highway=steps` 需自架 OSRM 並改 foot profile,公開 demo 伺服器無法指定,屬已知限制。

### 排序

**`species_score` 的組成(分數越高越前面):**

```
species_score = 類別基礎分(物種×類別)
              + 棲地/天氣/回報調整(見下表)
              + OSM 評分 × 0.4         (5 星約 +2.0)
              + 距離加權               (越近分數越高，見下)
```

**距離加權**(`WALK_WEIGHT = 2.0`):
```
距離加權 = (最長步行時間 − 實際步行時間) / 最長步行時間 × 2.0
```
即:就在原地(0 分鐘)得滿額 +2.0,走到時間上限則 +0。例如上限 15 分鐘、某點走 5 分鐘 → `(15−5)/15 × 2 = +1.33`。

**棲地/天氣/回報調整:**

| 規則 | 影響物種 | 調整 |
|------|----------|------|
| 無遮蔭 | 守宮 | −3.0(高溫再 −3.5、UV≥6 再 −2.0) |
| 非平坦地形(森林/校園/商店) | 陸龜 | −3.0 |
| 便利商店(吵雜) | 鬃獅蜥 | −3.0 |
...
**顯示用的 1–5 適合度**:把上面的 `species_score`(實際範圍約 −2 ~ +6)線性映射到 **1.0 ~ 5.0**(`_suitability_stars`)。所以「適合度」同時反映了**棲地契合 + 距離遠近 + 社群回報**。

排序:先依 `species_score` 由高到低,再以 `walk_min` 短者優先,最後**只取前 20 名**回傳。

> 設計理念：守宮夜行性、怕高溫高 UV;陸龜需平坦草地;鬃獅蜥需安靜、怕犬隻;且大家都偏好「近一點」。
