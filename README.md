...
每個物種有「只能前往」的棲地限制,不符合的地點會被**直接排除**,再對通過的地點做評分排序:

| 物種 | 只允許 | 原因 |
|------|--------|------|
| 🦎 守宮/蜥蜴(夜行性) | **有遮蔭** 的地點(遮蔭公園、林蔭步道) | 避開高溫與強光 |
| 🐢 陸龜(平坦地形) | **草地、平坦公園** | 需平坦無障礙,排除崎嶇森林與繁忙商店 |
| 🦕 鬃獅蜥(安靜區域) | 公園、草地、森林、校園(**排除便利商店**) | 偏好人少、低流量的開闊空間 |

> ⚠️ **階梯限制**:陸龜「不可走階梯」目前僅以「平坦地點類別」近似達成。真正在路徑層級避開 `highway=steps` 需自架 OSRM 並改 foot profile,公開 demo 伺服器無法指定,屬已知限制。

### 排序

通過上述硬性規則後,排序鍵（`routeFilter.filter_and_rank`）依序為：
